  ...
  export default {
    ...
    methods: {
      async fetchUser() {
        ...
        if (this.steamUserId.length === 17) {
          const apiUrl = `https://localhost:7257/api/SteamUser/GetByID/${this.steamUserId}`;
          this.isLoading = true;
      ...
      addUser() {
        if (this.steamUserId.length === 17 && !this.searchedUsers.includes(this.steamUserId)) {
